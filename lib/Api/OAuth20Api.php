...
class OAuth20Api
{
     ...
     *
     * @param  string $grant_type Value MUST be set to \&quot;authorization_code\&quot; (required)
     * @param  string $client_id client_id (required)
     * @param  string $client_secret client_secret (required)
     * @param  string $redirect_uri redirect_uri (required)
     * @param  string $code The code received in the query string when redirected from authorization page (optional)
     * @param  string $refresh_token The refresh_token received in the query string when redirected from authorization page (optional)
     *
     * @throws \Meli\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Meli\Model\Token
     */
    public function getToken($grant_type, $client_id, $client_secret, $redirect_uri, $code = null, $refresh_token = null)
    {
     ...
     * @throws \Meli\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Meli\Model\Token, HTTP status code, HTTP response headers (array of strings)
     */
    public function getTokenWithHttpInfo($grant_type, $client_id, $client_secret, $redirect_uri, $code = null, $refresh_token = null)
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Meli\Model\Token' === '\SplFileObject') {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Meli\Model\Token', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\Meli\Model\Token';
            $responseBody = $response->getBody();
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getTokenAsync($grant_type, $client_id, $client_secret, $redirect_uri, $code = null, $refresh_token = null)
    {
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getTokenAsyncWithHttpInfo($grant_type, $client_id, $client_secret, $redirect_uri, $code = null, $refresh_token = null)
    {
        $returnType = '\Meli\Model\Token';
        $request = $this->getTokenRequest($grant_type, $client_id, $client_secret, $redirect_uri, $code, $refresh_token);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function getTokenRequest($grant_type, $client_id, $client_secret, $redirect_uri, $code = null, $refresh_token = null)
    {
        ...
        if ($redirect_uri === null || (is_array($redirect_uri) && count($redirect_uri) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $redirect_uri when calling getToken'
            );
        }
        ...
        if ($refresh_token !== null) {
            if('form' === 'form' && is_array($refresh_token)) {
                foreach($refresh_token as $key => $value) {
                    $queryParams[$key] = $value;
                }
            }
            else {
